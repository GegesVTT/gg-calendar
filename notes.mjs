...
export class CalendarEngine {
  ...
  /**
   * Phase of every moon defined in config.moons at a given worldTime.
   * config.moons: [{ name, cycleDays, offset }]
   *  - cycleDays: full cycle length in days (may be fractional, e.g. 29.53)
   *  - offset: day offset so phase 0 (new moon) lands on a lore-correct date
   * Returns [{ name, phaseIndex (0-7), phaseKey, glyph, fraction }]
   * Phase 0 = new, 4 = full.
   */
  moonPhases(t) {
    const moons = this.config.moons ?? [];
    if (!moons.length) return [];
    const dayOffset = Math.floor(t / DAY_SECONDS);
    return moons.map(m => {
      const cycle = m.cycleDays;
      const pos = (((dayOffset - (m.offset ?? 0)) % cycle) + cycle) % cycle;
      const fraction = pos / cycle;
      const phaseIndex = Math.round(fraction * 8) % 8;
      return {
        name: m.name,
        phaseIndex,
        phaseKey: CalendarEngine.MOON_PHASES[phaseIndex],
        glyph: CalendarEngine.MOON_GLYPHS[phaseIndex],
        fraction
      };
    });
  }

  static MOON_PHASES = ["new", "waxingCrescent", "firstQuarter", "waxingGibbous", "full", "waningGibbous", "lastQuarter", "waningCrescent"];
  static MOON_GLYPHS = ["🌑", "🌒", "🌓", "🌔", "🌕", "🌖", "🌗", "🌘"];
}
